class Parser{
	...
	{       Exp exp1, exp2;
		...
		if(tokenCorrente.token == TokenType.NUM){
			int numero;
			if(tokenCorrente.lexema != '\0'){
				numero = Integer.parseInt(tokenCorrente.lexema+"");
			}
			else {
				numero = Integer.parseInt(tokenCorrente.numero+"");
			}

			
			return new Num(numero);
		}
